...
function primsAlgorithm() {
  ...
  do {
    frontier.push.apply(
      frontier,
      getFrontier(lastestDiscovered, discovered, frontier)
    );
...
function eraseBlock(frontierChoosen, discovered) {
  ...
  //Top
  if (
    currentIndexY + 2 < numberOfTilesVertical &&
    contains(discovered, [currentIndexX, currentIndexY + 2]) &&
    !contains(choiceChoosen, [currentIndexX, currentIndexY + 2])
  ) {
  ...
  //Right
  if (
    currentIndexX + 2 < numberOfTilesHorizontal &&
    contains(discovered, [currentIndexX + 2, currentIndexY]) &&
    !contains(choiceChoosen, [currentIndexX + 2, currentIndexY])
  ) {
  ...
  mazeBlocksToErase.push(
    blocks[
      getGrid2DIndex(
        (choice[0] + frontierChoosen[0]) / 2,
        (choice[1] + frontierChoosen[1]) / 2
      )
    ]
  );
  blocks[
...
function getFrontier(lastestDiscovered, discovered, allFrontier) {
  ...
  if (
    currentIndexY + 2 < numberOfTilesVertical &&
    !contains(discovered, [currentIndexX, currentIndexY + 2]) &&
    !contains(allFrontier, [currentIndexX, currentIndexY + 2]) &&
    !contains(frontier, [currentIndexX, currentIndexY + 2])
  ) {
  ...
  if (
    currentIndexX + 2 < numberOfTilesHorizontal &&
    !contains(discovered, [currentIndexX + 2, currentIndexY]) &&
    !contains(allFrontier, [currentIndexX + 2, currentIndexY]) &&
    !contains(frontier, [currentIndexX + 2, currentIndexY])
  ) {
...
function mazeGridInit() {
  for (var i = 0; i < numberOfTilesVertical; i++) {
    for (var j = 0; j < numberOfTilesHorizontal; j++) {
      if (
        (i % 2 == 0 || j % 2 == 0) &&
        !blocks[getGrid2DIndex(i, j)].isObstacle
      ) {
